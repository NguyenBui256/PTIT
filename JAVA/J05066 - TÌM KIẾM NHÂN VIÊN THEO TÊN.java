...
public class Main {
    public static void main(String[] args) {
        ...
        for(int i = 1; i <= n; i++) {
            ...
            if((chucTmp.equals("GD") && (mp.getOrDefault("GD", 0) == 1 || (Integer.parseInt(x.soHieu) > 1))
                    || (chucTmp.equals("TP") && (mp.getOrDefault("TP", 0) == 3 || Integer.parseInt(x.soHieu) > 3))
                    || (chucTmp.equals("PP") && (mp.getOrDefault("PP", 0) == 3 || Integer.parseInt(x.soHieu) > 3)))) {
                chucTmp = "NV";
            } else {
        ...
        for(int i = 1; i <= m; i++) {
            ...
            for(NhanVien x : ls) {
                if(x.ten.toLowerCase().contains(query)) {
